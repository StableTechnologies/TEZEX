...
FA2 = sp.io.import_script_from_url("file:Fa2.py", name="FA2")
...
"""
Util to handle fa2 batch transfer calls
"""
...
Swap record - 
    ...
    value(nat): value of the swap in fa2 tokens
...
Contract Storage -
    ...
    fa2(address): fa2 contract address
    tokenID(nat): fa2 token id
    active(bool): contract state [true:active, false:inactive]
    swaps(big_map(bytes,Swap)): map of hashed secrets and swap details
"""


class FA2Swap(sp.Contract):
    def __init__(self, _admin, _fa2, _tokenID):
        self.init(admin=_admin, reward=sp.as_nat(15), fa2=_fa2, tokenID=_tokenID, active=sp.bool(False),
                  swaps=sp.big_map(tkey=sp.TBytes, tvalue=Swap))
        self.transfer = Batch_transfer()
    ...
    """
        ...
        args:
            ...
            _amount: amount of fa2 tokens exchanged in the swap
            _refundTimestamp: unix time(sec) after which the swap expires
    """
    ...
    def redeem(self, _hashedSecret, _secret):
        ...
        contract = sp.contract(self.transfer.get_type(), self.data.fa2, "transfer").open_some()                    
        sp.transfer([sp.record(from_=sp.self_address, txs=[sp.record(to_=self.data.swaps[_hashedSecret].participant, token_id=self.data.tokenID, amount=self.data.swaps[_hashedSecret].value)])],sp.mutez(0), contract)
    ...
    def refund(self, _hashedSecret):
        ...
        contract = sp.contract(self.transfer.get_type(), self.data.fa2, "transfer").open_some()                    
        sp.transfer([sp.record(from_=sp.self_address, txs=[sp.record(to_=self.data.swaps[_hashedSecret].initiator, token_id=self.data.tokenID, amount=self.data.swaps[_hashedSecret].value)])],sp.mutez(0), contract)
        del self.data.swaps[_hashedSecret]
