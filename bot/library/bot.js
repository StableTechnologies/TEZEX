...
module.exports = class Bot {
    ...
    async init(ethConfig, tezosConfig, volume) {
        try {
            console.log('\nInitializing Bot...');
            this.volume = {
                usdc: parseInt(volume.usdc, 10),
                usdtz: parseInt(volume.usdtz, 10)
            };
            const web3 = new Web3(
    ...
    async start() {
        ...
        const ops = [];
        if (allowances[0] !== this.volume.usdtz) ops.push(this.usdtz.approveToken(this.volume.usdtz));
        if (allowances[1] !== this.volume.usdc) ops.push(this.usdc.approveToken(this.volume.usdc));
        ...
        case 3: {
            try {
                if (type === 2) {
                    ...
                    if (parseInt(allowance) !== this.volume.usdc) await this.usdc.approveToken(this.volume.usdc);
                } else {
                    ...
                    if (parseInt(allowance) !== this.volume.usdtz) await this.usdtz.approveToken(this.volume.usdtz);
                }
            } catch (err) {
    ...
    monitorRefunds() {
        const run = async () => {
            ...
            for (const key in this.usdcSwaps) {
                if (this.usdcSwaps[key].state === 4 && this.usdcSwaps[key].refundTime <= Date.now() / 1000) {
                    try {
            ...
            for (const key in this.usdtzSwaps) {
                if (this.usdtzSwaps[key].state === 4 && this.usdtzSwaps[key].refundTime <= Date.now() / 1000) {
                    try {
