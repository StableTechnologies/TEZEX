...
export const dexterPoolStorageMap = {
	coinBalancePath: "$.args[4].int",
	tokenBalancePath: "$.args[3].int",
	liquidityBalancePath: "$.args[1].args[2].int",
};
export const granadaPoolStorageMap = {
	coinBalancePath: "$.args[1].int",
	tokenBalancePath: "$.args[0].int",
	liquidityBalancePath: "$.args[2].int",
};
export const vortexPoolStorageMap = {
	coinBalancePath: "$.args[1].int",
	tokenBalancePath: "$.args[0].int",
	liquidityBalancePath: "$.args[2].int",
};
...
export const tokenPoolMap = {
	...
	KT1PWx2mnDueood7fEmfbBDKx1D9BAnnXitn: {
		granadaPool: "KT1TxqZ8QtKvLu3V3JH7Gx58n7Co8pgtpQU5",
		quipuPool: "KT1WBLrLE2vG8SedBqiSJFm4VVAZZBytJYHc",
	}, // tzbtc
	...
	KT1AEfeckNbdEYwaMKkytBwPJPycz7jdSGea: {
		dexterPool: "",
		quipuPool: "KT1BMEEPX7MWzwwadW3NCSZe9XGmFJ7rs7Dr",
	}, // stkr
	...
	KT1G1cCRNBgQ48mVDjopHjEmTN5Sbtar8nn9: {
		dexterPool: "",
		quipuPool: "KT1BgezWwHBxA9NrczwK9x3zfgFnUkc7JJ4b",
	}, // heh
	...
	KT1REEb5VxWRjcHm5GzDMwErMmNFftsE5Gpf: {
		dexterPool: "",
		quipuPool: "KT1KFszq8UFCcWxnXuhZPUyHT9FK3gjmSKm6",
	}, // usds
	...
	KT1AxaBxkFLCUi3f8rdDAAxBKHfzY8LfKDRA: {
		dexterPool: "",
		quipuPool: "KT1WtFb1mTsFRd1n1nAYMdrE2Ud9XREz5hjK",
	}, // QLkUSD
	...
	KT1Wa8yqRBpFCusJWgcQyjhRz7hUQAmFxW7j: {
		dexterPool: "",
		quipuPool: "KT1Q93ftAUzvfMGPwC78nX8eouL1VzmHPd4d",
	}, // FLAME
	KT1JkoE42rrMBP9b2oDhbx6EUr26GcySZMUH: {
		dexterPool: "",
		quipuPool: "KT1NEa7CmaLaWgHNi6LkRi5Z1f4oHfdzRdGA",
	}, // kDAO
	...
	"KT18fp5rcTW7mbWDmzFwjLDUhs5MeJmagDSZ+19": {
		dexterPool: "",
		quipuPool: "KT1DksKXvCBJN7Mw6frGj6y6F3CbABWZVpj1",
	}, // wwBTC
	"KT18fp5rcTW7mbWDmzFwjLDUhs5MeJmagDSZ+17": {
		dexterPool: "",
		quipuPool: "KT1U2hs5eNdeCpHouAvQXGMzGFGJowbhjqmo",
	}, // wUSDC
	"KT18fp5rcTW7mbWDmzFwjLDUhs5MeJmagDSZ+1": {
		dexterPool: "",
		quipuPool: "KT1UMAE2PBskeQayP5f2ZbGiVYF7h8bZ2gyp",
	}, // wBUSD
	"KT18fp5rcTW7mbWDmzFwjLDUhs5MeJmagDSZ+11": {
		dexterPool: "",
		quipuPool: "KT1RsfuBee5o7GtYrdB7bzQ1M6oVgyBnxY4S",
	}, // wMATIC
	"KT18fp5rcTW7mbWDmzFwjLDUhs5MeJmagDSZ+10": {
		dexterPool: "",
		quipuPool: "KT1Lpysr4nzcFegC9ci9kjoqVidwoanEmJWt",
	}, // wLINK
	KT193D4vozYnhGJQVtw7CoxxqphqUEEwK6Vb: {
		dexterPool: "",
		quipuPool: "KT1X3zxdTzPB9DgVzA3ad6dgZe9JEamoaeRy",
	}, // QUIPU
	KT1KPoyzkj82Sbnafm6pfesZKEhyCpXwQfMc: {
		dexterPool: "",
		quipuPool: "KT1BweorZK1CJDEu76SyKcxfzeiAxip73Kot",
	}, // fDAO
	KT1Kyc366SrSJ1camxByr363F9WAiHAaReMv: {
		dexterPool: "",
		quipuPool: "KT1EVNuwjj7GijvVaxR7Vd9mejy4V6toZkeK",
	}, // sexp
...
export function isTradeable(tokenAddress: string, tokenIndex?: number) {
	if (tokenIndex !== undefined) {
		return Object.keys(tokenPoolMap).includes(
			`${tokenAddress}+${tokenIndex}`
		);
	}
...
export async function sendDexterBuy(
	...
	try {
		if (walletInfo.toolkit) {
			...
			const operation = await lbcontract.methods
				.xtzToToken(
					keyStore.publicKeyHash,
					notional,
					expiration
...
export async function sendDexterSell(
	...
	const nextCounter =
		(await TezosNodeReader.getCounterForAccount(
			tezosNode,
			keyStore.publicKeyHash
		)) + 1;
	...
	if (tokenAddress === "KT1PWx2mnDueood7fEmfbBDKx1D9BAnnXitn") {
		...
		sellParams = `{ "prim": "Pair", "args": [ { "prim": "Pair", "args": [ { "string": "${
			keyStore.publicKeyHash
		}" }, { "string": "${
			keyStore.publicKeyHash
	...
	try {
		...
		const r = await TezosNodeWriter.sendOperation(
			tezosNode,
			opGroup,
			signer
		);
...
export async function sendQuipuSell(
	...
	if (tokenIndex > -1) {
		selectedToken = knownTokenContracts.filter(
			(t) =>
	...
	if (
		selectedToken.kind === TokenKind.tzip12 ||
		selectedToken.kind === TokenKind.objkt
	) {
		...
		approveOp = constructFA1ApprovalOperation(
			keyStore.publicKeyHash,
			nextCounter,
			{ fee: 0, gas: 0, storage: 0 },
			tokenAddress,
...
export function getTokenToCashExchangeRate(
	...
	const f = dm.remainder
		.multiply(bigInt(10 ** tokenDecimals))
		.divide(bigInt(tokenAmount));

	return {
		cashAmount: cashAmount.toJSNumber(),
		rate: parseFloat(
			`${dm.quotient.toJSNumber()}.${f.toJSNumber()}`
		),
	};
}
...
export function getTokenToCashInverse(
	...
	const d = bigInt(tokenBalance)
		.multiply(bigInt(exchangeMultiplier))
		.subtract(
			bigInt(tokenAmount).multiply(bigInt(exchangeMultiplier))
		);

	const amount = n.divide(d);
	const rate = amount.divmod(bigInt(tokenAmount));
	const ff = rate.remainder
		.multiply(bigInt(10 ** tokenDecimals))
		.divide(bigInt(tokenAmount));

	return {
		cashAmount: amount.toJSNumber(),
		rate: parseFloat(
			`${rate.quotient.toJSNumber()}.${ff.toJSNumber()}`
		),
	};
}

export function calcTokenLiquidityRequirement(
	cashDeposit: string,
	tokenBalance: string,
	cashBalance: string
): number {
	return bigInt(cashDeposit)
		.multiply(tokenBalance)
		.divide(cashBalance)
		.toJSNumber();
}

export function calcCashLiquidityRequirement(
	tokenDeposit: string,
	tokenBalance: string,
	cashBalance: string
): number {
	return bigInt(tokenDeposit)
		.multiply(bigInt(cashBalance))
		.divide(bigInt(tokenBalance))
		.toJSNumber();
}

export function calcPoolShare(
	poolShare: string,
	partBalance: string,
	liquidityBalance: string
): number {
	return bigInt(partBalance)
		.multiply(poolShare)
		.divide(liquidityBalance)
		.toJSNumber();
}

export function calcProposedShare(
	cashDeposit: string,
	cashBalance: string,
	liquidityBalance: string
): number {
	return bigInt(cashDeposit)
		.multiply(liquidityBalance)
		.divide(cashBalance)
		.toJSNumber();
}
...
export async function getPoolState(
	...
	const tokenBalance = JSONPath({
		path: storageMap.tokenBalancePath,
		json: storageResult,
	})[0];
	const xtzBalance = JSONPath({
		path: storageMap.coinBalancePath,
		json: storageResult,
	})[0];
	const liquidityBalance = JSONPath({
		path: storageMap.liquidityBalancePath,
		json: storageResult,
	})[0];
