...
token ID, script-code hash, integration owner, and incident channel. No
...
hash with the manifest pin at least once per monitoring interval. Missing
indexer data, RPC disagreement, or a code-hash mismatch is itself an alert.
...
2. Record the triggering operation, block, observed code hash, token control
...
token ID, current script-code hash, resolved alert, successful transfer/balance
