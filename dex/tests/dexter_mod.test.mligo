...
//     Util.assert_token_balance lqt_orig.taddr test_name (Util.src ()) 1000000n;
//     Util.assert_accumulated_fee dex_orig.taddr test_name 0mutez
//   end
...
// (* With fee: only net XTZ enters the pool, fee is accumulated *)
...
//     Util.assert_accumulated_fee dex_orig.taddr test_name 5000mutez
//   end

// (* minTokensBought is checked against the post-fee amount *)
...
// (* With fee: user receives less XTZ, fee is accumulated, xtzPool decreases by gross *)
...
//   begin
//     Util.assert_dex_state dex_orig.taddr test_name 667335mutez 1500000n 1000000n;
//     Util.assert_accumulated_fee dex_orig.taddr test_name 3326mutez
//   end

// (* minXtzBought is checked against the post-fee amount *)
...
// (*****************************************************************************)
// (* claimProtocolFee tests                                                    *)
// (*****************************************************************************)
// let test_claim_protocol_fee =
//   let test_name = "test_claim_protocol_fee" in
//   let (dex_orig, _, _) = Util.setup_full_dex_with_fee 50n in
//   let swap_param : DexterMod.Dexter.xtz_to_token =
//     { to_ = Util.src (); minTokensBought = 1n; deadline = Util.future } in
//   // Swap 1 tez; fee = floor(1_000_000 * 50 / 10000) = 5_000 mutez
//   let _ : nat =
//     Test.Typed_address.transfer_exn dex_orig.taddr (XtzToToken swap_param) 1tez in
//   let () = Util.assert_accumulated_fee dex_orig.taddr test_name 5000mutez in
//   let balance_before = Test.Typed_address.get_balance dex_orig.taddr in
//   let _ : nat =
//     Test.Typed_address.transfer_exn dex_orig.taddr (ClaimProtocolFee ()) 0tez in
//   let storage = Test.Typed_address.get_storage dex_orig.taddr in
//   let balance_after = Test.Typed_address.get_balance dex_orig.taddr in
//   begin
//     Assert.assert (storage.accumulated_protocol_fee = 0mutez);
//     Assert.assert (balance_before = balance_after + 5000mutez)
//   end

// let test_claim_protocol_fee_multiple_swaps =
//   let test_name = "test_claim_protocol_fee_multiple_swaps" in
//   let (dex_orig, _, _) = Util.setup_full_dex_with_fee 50n in
//   let xtz_swap : DexterMod.Dexter.xtz_to_token =
//     { to_ = Util.src (); minTokensBought = 1n; deadline = Util.future } in
//   // First swap: 1 tez — fee = 5_000 mutez
//   let _ : nat =
//     Test.Typed_address.transfer_exn dex_orig.taddr (XtzToToken xtz_swap) 1tez in
//   // Second swap: 500_000 mutez — fee = 2_500 mutez
//   let _ : nat =
//     Test.Typed_address.transfer_exn dex_orig.taddr (XtzToToken xtz_swap) 500000mutez in
//   let () = Util.assert_accumulated_fee dex_orig.taddr test_name 7500mutez in
//   let _ : nat =
//     Test.Typed_address.transfer_exn dex_orig.taddr (ClaimProtocolFee ()) 0tez in
//   let storage = Test.Typed_address.get_storage dex_orig.taddr in
//   Assert.assert (storage.accumulated_protocol_fee = 0mutez)

// let test_claim_protocol_fee_error_not_recipient =
//   let test_name = "test_claim_protocol_fee_error_not_recipient" in
//   let (dex_orig, _, _) = Util.setup_full_dex_with_fee 50n in
//   let swap_param : DexterMod.Dexter.xtz_to_token =
//     { to_ = Util.src (); minTokensBought = 1n; deadline = Util.future } in
//   let _ : nat =
//     Test.Typed_address.transfer_exn dex_orig.taddr (XtzToToken swap_param) 1tez in
//   let () = Test.State.set_source (Util.other ()) in
//   let result = Test.Typed_address.transfer dex_orig.taddr (ClaimProtocolFee ()) 0tez in
//   Util.assert_error
//     test_name
//     DexterMod.Dexter.error_ONLY_RECIPIENT_CAN_CLAIM_PROTOCOL_FEE
//     result

// let test_claim_protocol_fee_error_nothing_to_claim =
//   let test_name = "test_claim_protocol_fee_error_nothing_to_claim" in
//   let (dex_orig, _, _) = Util.setup_full_dex () in // fee = 0
//   let result = Test.Typed_address.transfer dex_orig.taddr (ClaimProtocolFee ()) 0tez in
//   Util.assert_error test_name DexterMod.Dexter.error_NO_PROTOCOL_FEE_TO_CLAIM result

// let test_claim_protocol_fee_error_amount =
//   let test_name = "test_claim_protocol_fee_error_amount" in
//   let (dex_orig, _, _) = Util.setup_full_dex_with_fee 50n in
//   let swap_param : DexterMod.Dexter.xtz_to_token =
//     { to_ = Util.src (); minTokensBought = 1n; deadline = Util.future } in
//   let _ : nat =
//     Test.Typed_address.transfer_exn dex_orig.taddr (XtzToToken swap_param) 1tez in
//   let result = Test.Typed_address.transfer dex_orig.taddr (ClaimProtocolFee ()) 1tez in
//   Util.assert_error test_name DexterMod.Dexter.error_AMOUNT_MUST_BE_ZERO result

// let test_claim_protocol_fee_error_updating_pool =
//   let test_name = "test_claim_protocol_fee_error_updating_pool" in
//   let (dex_orig, _, _) = Util.setup_dex_with_updating_pool () in
//   let result = Test.Typed_address.transfer dex_orig.taddr (ClaimProtocolFee ()) 0tez in
//   Util.assert_error
//     test_name
...
// let test_claim_protocol_fee_double_claim =
//   let test_name = "test_claim_protocol_fee_double_claim" in
//   let (dex_orig, _, _) = Util.setup_full_dex_with_fee 50n in
//   let swap_param : DexterMod.Dexter.xtz_to_token =
//     { to_ = Util.src (); minTokensBought = 1n; deadline = Util.future } in
//   let _ : nat =
...
// (* Changing recipient takes effect immediately *)
// let test_claim_after_recipient_change =
//   let test_name = "test_claim_after_recipient_change" in
//   let (dex_orig, _, _) = Util.setup_full_dex_with_fee 50n in
//   let swap_param : DexterMod.Dexter.xtz_to_token =
//     { to_ = Util.src (); minTokensBought = 1n; deadline = Util.future } in
//   let _ : nat =
...
//   let param : address = Util.other () in
//   let _ : nat =
//     Test.Typed_address.transfer_exn dex_orig.taddr (SetProtocolFeeRecipient param) 0tez in
...
//   // fee = 100 bp; gross = 332_888; net = 329_560
//   let (dex_orig, _, _) = Util.setup_full_dex_with_fee 100n in
//   let dex_address = Test.Typed_address.to_address dex_orig.taddr in
//   let view_result : nat option =
//     Tezos.View.call "quote_token_to_tez" 500000n dex_address in
