...
let test_fa2_setup =
  ...
  let (dex_orig, lqt_orig, tok_orig) = Util.setup_full_dex_with_fees 25n 5n in
  begin
...
(* FA2 pool with target mainnet fees: 25 bp LP + 5 bp protocol *)
let test_fa2_xtz_to_token_target_fees =
  let test_name = "test_fa2_xtz_to_token_target_fees" in
  let (dex_orig, _, tok_orig) = Util.setup_full_dex_with_fees 25n 5n in
  ...
  begin
    Util.assert_dex_state dex_orig.taddr test_name 1999500mutez 500751n 1000000n;
    Util.assert_fa2_balance tok_orig.taddr test_name (Util.src ()) 1000499249n;
    Util.assert_accumulated_fee_xtz dex_orig.taddr test_name 500mutez
  end

let test_fa2_view_get_fee_bp_target_fees =
  let (dex_orig, _, _) = Util.setup_full_dex_with_fees 25n 5n in
